...
-- pipeline (backend `/disease/screen`). The frontend degrades gracefully when
-- this table is absent: results still render, they just are not stored.
--
...
-- reads her own. Nobody edits or deletes a recorded screening from the browser.
...
create policy "doctors record screenings for their patients"
  ...
  to authenticated
  with check (doctor_id = auth.uid() and public.doctor_treats(patient_id));
