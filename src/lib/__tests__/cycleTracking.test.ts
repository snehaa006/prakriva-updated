...
describe("analyseCycles", () => {
  ...
  // Periods stop in pregnancy, so the gap is the pregnancy — reporting it as
  // amenorrhoea would be a false finding generated by an entirely normal
  // event, shown to the patient least able to shrug it off.
  it("does not read a pregnant patient's gap as missed periods", () => {
    const entries = [period("2026-01-01"), period("2026-01-29")];
    const analysis = analyseCycles(entries, [], {
      today: "2026-08-01",
      isPregnant: true,
    });

    expect(analysis.flags.map((f) => f.key)).toEqual(["pregnant"]);
    expect(analysis.isOverdue).toBe(false);
    expect(analysis.predictedNextStart).toBeNull();
    expect(analysis.regularity).not.toBe("highly-irregular");
  });

  it("keeps the history itself while pregnant", () => {
    const entries = [period("2026-01-01"), period("2026-01-29")];
    const analysis = analyseCycles(entries, [], {
      today: "2026-08-01",
      isPregnant: true,
    ...
    expect(analysis.cycleCount).toBe(1);
    expect(analysis.averageCycleLength).toBe(28);
    expect(analysis.lastPeriodStart).toBe("2026-01-29");
  });

  it("still accepts a bare date string for the third argument", () => {
    const analysis = analyseCycles(regular, [], "2026-04-01");
    expect(analysis.averageCycleLength).toBe(28);
  });
